// Updated membershipPages to work with separate auth page by replacing hideLoginGate() calls with window.location.href = './index.html';
...
function showAdminPage() {
    ...
    const contentTitle = document.querySelector('.selected-workspace-name');
    ...
    switch(type) {
        case 'users':
            iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M17 21v-2a4 4 0 0 0-4-4H5a4```javascript
 4 0 0-4 4v2"></path>
...
function showAddUserModal() {
    ...
    modal.innerHTML = `
        <div style="background-color: var(--card-color); border-radius: 8px; width: 500px; max-width: 90%; padding: 20px;">
            <h2 style="margin-top: 0; margin-bottom: 20px;">Add New User</h2>
            <form id="addUserForm">
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Full Name *</label>
                    <input type="text" id="userName" required style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                </div>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Email *</label>
                    <input type="email" id="userEmail" required style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                </div>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Role *</label>
                    <select id="userRole" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
    ...
    document.getElementById('addUserForm').addEventListener('submit', function(e) {
        ...
        let users = JSON.parse(localStorage.getItem('kb_users') || '[]');
        const newUser = {
            id: 'user-' + Date.now(),
            name: name,
            email: email,
        ...
        // Refresh admin page if currently visible
        const currentTitle = document.querySelector('.selected-workspace-name');
        if (currentTitle && currentTitle.textContent === 'Admin Dashboard') {
            showAdminPage();
        }
    });
}

// Function to show Upgrade Plan Modal
function showUpgradePlanModal() {
    ...
    modal.innerHTML = `
        <div style="background-color: var(--card-color); border-radius: 8px; width: 600px; max-width: 90%; padding: 20px;">
            <h2 style="margin-top: 0; margin-bottom: 20px;">Upgrade Your Plan</h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div style="border: 1px solid var(--border-color); border-radius: 8px; padding: 20px; text-align: center;">
                    <h3>Pro Plan</h3>
                    <div style="font-size: 24px; font-weight: bold; margin: 10px 0;">$9/month</div>
                    ...
                    <button onclick="upgradeToPlan('pro')" class="btn btn-primary" style="width: 100%;">Select Pro</button>
                </div>
                <div style="border: 2px solid var(--primary-color); border-radius: 8px; padding: 20px; text-align: center;">
                    <h3>Business Plan</h3>
                    <div style="font-size: 24px; font-weight: bold; margin: 10px 0;">$29/month</div>
                    <ul style="text-align: left; margin: 15px 0;">
                    ...
                    <button onclick="upgradeToPlan('business')" class="btn btn-primary" style="width: 100%;">Select Business</button>
                </div>
            </div>
            <div style="text-align: center;">
                <button onclick="this.closest('.modal-overlay').remove()" class="btn btn-outline">Cancel</button>
            </div>
        </div>
    `;
...
function showPaymentConfirmationModal(plan, onSuccess) {
    const planDetails = {
        pro: { name: 'Pro Plan', price: '$9', features: ['Up to 10 books', 'Advanced analytics', 'Team collaboration', 'Priority support'] },
        business: { name: 'Business Plan', price: '$29', features: ['Unlimited books', 'Advanced security', 'Custom branding', 'API access'] }
    };
    ...
    modal.innerHTML = `
        <div style="background-color: var(--card-color); border-radius: 8px; width: 500px; max-width: 90%; padding: 30px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3);">
            <h2 style="margin-top: 0; margin-bottom: 20px;">Confirm Your Upgrade</h2>

            <div style="background-color: var(--hover-color); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <h3 style="margin: 0;">${selectedPlan.name}</h3>
                    <div style="font-size: 24px; font-weight: bold; color: var(--primary-color);">${selectedPlan.price}/month</div>
                </div>
                <ul style="margin: 0; padding-left: 20px;">
                    ${selectedPlan.features.map(feature => `<li style="margin-bottom: 5px;">${feature}</li>`).join('')}
                </ul>
            </div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" placeholder="1234 5678 9012 3456" style="width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                </div>
                <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                    <div style="flex: 1;">
                        <label style="display: block; margin-bottom: 5px;">Expiry Date</label>
                        <input type="text" placeholder="MM/YY" style="width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                    </div>
                    <div style="flex: 1;">
                        <label style="display: block; margin-bottom: 5px;">CVV</label>
                        <input type="text" placeholder="123" style="width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                    </div>
                </div>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Cardholder Name</label>
                    <input type="text" placeholder="John Doe" style="width: 100%; padding: 12px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                </div>
            </div>

            <div style="background-color: rgba(100, 197, 255, 0.1); padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid rgba(100, 197, 255, 0.3);">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px;">
                <button onclick="this.closest('.modal-overlay').remove()" class="btn btn-outline">Cancel</button>
                <button onclick="processPayment('${plan}', onSuccess)" class="btn btn-primary">Complete Upgrade</button>
            </div>
    ...
    // Define processPayment function globally for the onclick handler
    window.processPayment = function(plan, callback) {
...
function showInviteMemberModal() {
    ...
    modal.innerHTML = `
        <div style="background-color: var(--card-color); border-radius: 8px; width: 500px; max-width: 90%; padding: 20px;">
            <h2 style="margin-top: 0; margin-bottom: 20px;">Invite Team Member</h2>
            <form id="inviteForm">
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Email Address *</label>
                    <input type="email" id="inviteEmail" required style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                </div>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Role *</label>
                    <select id="inviteRole" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);">
                ...
                <div style="margin-bottom: 15px;">
                    <label style="display: block; margin-bottom: 5px;">Personal Message (optional)</label>
                    <textarea id="inviteMessage" rows="3" style="width: 100%; padding: 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--background-color); color: var(--text-color);" placeholder="Welcome to our team!"></textarea>
                </div>
                <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px;">
                    <button type="button" onclick="this.closest('.modal-overlay').remove()" class="btn btn-outline">Cancel</button>
