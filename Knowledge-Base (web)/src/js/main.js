...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check if user should see login gate
    checkLoginGate();
...
// Check if login gate should be shown
function checkLoginGate() {
    const user = JSON.parse(localStorage.getItem('kb_current_user') || 'null');
    const skipLoginGate = localStorage.getItem('kb_skip_login_gate');
...
// Hide the login gate and show main app
...
function continueAsGuest() {
    localStorage.setItem('kb_skip_login_gate', 'true');
    hideLoginGate();
    showNotification('Welcome! You are using Knowledge Base as a guest.', 'info');
}
...
function setupEventListeners() {
    ...
    // Listen for content table updates to re-setup expand/collapse listeners
    document.removeEventListener('contentTableUpdated', handleContentTableUpdated);
    document.addEventListener('contentTableUpdated', handleContentTableUpdated);
    
    console.log("All event listeners setup complete");
}

// Handle content table updates
function handleContentTableUpdated() {
    console.log("Content table updated, refreshing event listeners");
    
    // Refresh tree event listeners when content is updated
...
function setupSidebarNavigation() {
    ...
    if (adminIcon) {
        adminIcon.addEventListener('click', function(e) {
            ...
            // Check for membershipPages functions first
            if (typeof membershipPages !== 'undefined' && typeof membershipPages.showAdminPage === 'function') {
                membershipPages.showAdminPage();
            } else if (typeof showAdminPage === 'function') {
                showAdminPage();
            } else {
                console.error('showAdminPage function not available');
                showNotification('Admin features loading...', 'info');
                // Try to call it directly from the global scope
                setTimeout(() => {
    ...
    if (teamIcon) {
        teamIcon.addEventListener('click', function(e) {
            ...
            // Check for membershipPages functions first
            if (typeof membershipPages !== 'undefined' && typeof membershipPages.showTeamPage === 'function') {
                membershipPages.showTeamPage();
            } else if (typeof showTeamPage === 'function') {
                showTeamPage();
            } else {
                console.error('showTeamPage function not available');
                showNotification('Team features loading...', 'info');
                // Try to call it directly from the global scope
                setTimeout(() => {
                    if (window.showTeamPage) {
                        window.showTeamPage();
                    }
                }, 100);
            }
...
function addNewArticle(parentId = null) {
    ...
    // Icon choices (same as create category modal)
    ...
    iconChoices.forEach(icon => {
        ...
        btn.style.backgroundColor = icon === selectedIcon ? 'var(--button-blue)' : 'var(--card-color)';
        btn.onclick = () => {
            selectedIcon = icon;
            Array.from(iconRow.children).forEach(b => b.style.backgroundColor = 'var(--card-color)');
    ...
    privacyRow.style.gap = '16px';
    privacyRow.style.margin = '10px 0';
    const privateLabel = document.createElement('label');
    privateLabel.style.display = 'flex';
    privateLabel.style.alignItems = 'center';
    const privateCheckbox = document.createElement('input');
    privateCheckbox.type = 'checkbox';
    privateCheckbox.style.marginRight = '6px';
    privateLabel.appendChild(privateCheckbox);
    privateLabel.appendChild(document.createTextNode('Private'));
    privacyRow.appendChild(privateLabel);
    newCatInputWrapper.appendChild(privacyRow);

    // Confirm button
    const newCatBtn = document.createElement('button');
    newCatBtn.type = 'button';
    newCatBtn.textContent = 'Add Category';
    newCatBtn.style.marginTop = '8px';
    newCatBtn.style.backgroundColor = 'var(--button-blue)';
    newCatBtn.style.color = '#fff';
    newCatBtn.style.border = 'none';
    newCatBtn.style.borderRadius = 'var(--border-radius)';
    newCatBtn.style.padding = '8px 16px';
    newCatBtn.style.cursor = 'pointer';
    newCatBtn.onclick = () => {
        ...
        bookCategories.get(selectedBookId).set(catId, newCategory);
        saveData();
        // Add to dropdown (before the create new option)
        const opt = document.createElement('option');
        opt.value = catId;
        opt.textContent = val;
    ...
    newCatInputWrapper.appendChild(newCatBtn);
    ...
    // Make all modals close when clicking outside
...
function addNewFile(parentId = null) {
    ...
    const filePrivateCheckbox = document.createElement('input');
    filePrivateCheckbox.type = 'checkbox';
    filePrivateCheckbox.id = 'filePrivate';
    filePrivateCheckbox.style.width = '16px';
    filePrivateCheckbox.style.height = '16px';
    
    const filePrivateLabel = document.createElement('label');
    filePrivateLabel.htmlFor = 'filePrivate';
    filePrivateLabel.textContent = 'Private';
    filePrivateLabel.style.fontSize = '14px';
    filePrivateLabel.style.cursor = 'pointer';
    
    privateOption.appendChild(filePrivateCheckbox);
    privateOption.appendChild(filePrivateLabel);
    ...
    pinnedLabel.textContent = 'Pin to top';
    pinnedLabel.style.fontSize = '14px';
    pinnedLabel.style.cursor = 'pointer';
    ...
    form.onsubmit = function(e) {
        ...
        const selectedIcon = iconChoices[0]; // Default icon
        const isPrivate = filePrivateCheckbox.checked;
        const pinnedToTop = pinnedCheckbox ? pinnedCheckbox.checked : false;
        
        // Get parent category ID
        let parentId = parentId_param;
        if (!parentId) {
            parentId = categorySelect.value;
            if (parentId === '__create_new__') {
                showNotification('Please create the category first or select an existing one', 'error');
                return false;
            }
        }
        
        // Create a new file ID
        const fileId = 'item-' + Date.now();
        ...
        reader.onload = function(event) {
            ...
            const fileObject = {
                id: fileId,
                name: fileName,
                type: uploadType, // 'file' or 'article' based on radio selection
                parentId: parentId,
                bookId: selectedBookId,
                icon: selectedIcon,
                isPrivate: isPrivate,
            ...
            // Get the book categories map or create if not exists
            if (!bookCategories.has(selectedBookId)) {
                bookCategories.set(selectedBookId, new Map());
            }
            
            // Add to book categories
            bookCategories.get(selectedBookId).set(fileId, fileObject);
            ...
            showNotification(`File "${fileName}" uploaded successfully as ${uploadType}`);
...
function showAnalyticsPage() {
    ...
    analyticsContent.style.padding = '20px 0';
    
    // Analytics Header
    const analyticsHeader = document.createElement('div');
    analyticsHeader.style.marginBottom = '30px';
    
    const analyticsTitle = document.createElement('h2');
    analyticsTitle.textContent = 'Analytics Dashboard';
    analyticsTitle.style.fontSize = '24px';
    analyticsTitle.style.fontWeight = '500';
    analyticsTitle.style.marginBottom = '10px';
    
    const analyticsSubtitle = document.createElement('p');
    analyticsSubtitle.textContent = 'Track your knowledge base performance and user engagement';
    analyticsSubtitle.style.color = 'var(--text-secondary)';
    analyticsSubtitle.style.fontSize = '15px';
    
    analyticsHeader.appendChild(analyticsTitle);
    analyticsHeader.appendChild(analyticsSubtitle);
    ...
    analyticsData.innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 10px 0; color: var(--text-color);">Total Views</h3>
                <div style="font-size: 32px; font-weight: bold; color: var(--primary-color);">1,247</div>
                <div style="font-size: 12px; color: var(--success-green);">+12% from last month</div>
            </div>
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 10px 0; color: var(--text-color);">Articles Read</h3>
                <div style="font-size: 32px; font-weight: bold; color: var(--primary-color);">89</div>
                <div style="font-size: 12px; color: var(--success-green);">+5% from last month</div>
            </div>
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 10px 0; color: var(--text-color);">Search Queries</h3>
                <div style="font-size: 32px; font-weight: bold; color: var(--primary-color);">156</div>
                <div style="font-size: 12px; color: var(--accent-color);">-2% from last month</div>
            </div>
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 10px 0; color: var(--text-color);">User Satisfaction</h3>
                <div style="font-size: 32px; font-weight: bold; color: var(--primary-color);">94%</div>
                <div style="font-size: 12px; color: var(--success-green);">+3% from last month</div>
            </div>
        </div>
        <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
            <h3 style="margin: 0 0 15px 0; color: var(--text-color);">Popular Articles</h3>
            <div style="color: var(--text-secondary);">Analytics data would be displayed here in a real implementation.</div>
        </div>
    `;
    
    analyticsContent.appendChild(analyticsHeader);
...
function showSettingsPage() {
    ...
    settingsContent.style.padding = '20px 0';
    
    // Settings Header
    const settingsHeader = document.createElement('div');
    settingsHeader.style.marginBottom = '30px';
    ...
    settingsSubtitle.textContent = 'Manage your knowledge base preferences and configuration';
    settingsSubtitle.style.color = 'var(--text-secondary)';
    settingsSubtitle.style.fontSize = '15px';
    
    settingsHeader.appendChild(settingsTitle);
    settingsHeader.appendChild(settingsSubtitle);
    
    // Settings sections
    const settingsData = document.createElement('div');
    settingsData.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 30px;">
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 15px 0; color: var(--text-color);">General Settings</h3>
            ...
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 15px 0; color: var(--text-color);">API Configuration</h3>
                <div style="display: flex; flex-direction: column; gap: 15px;">
                    <label style="display: flex; flex-direction: column; gap: 5px;">
                        <span>ChatGPT API Key</span>
                        <input type="password" placeholder="Enter your API key" style="padding: 8px; background: var(--background-color); border: 1px solid var(--border-color); border-radius: 4px; color: var(--text-color);">
                    </label>
                </div>
            </div>
            <div style="background-color: var(--card-color); padding: 20px; border-radius: 8px; border: 1px solid var(--border-color);">
                <h3 style="margin: 0 0 15px 0; color: var(--text-color);">Data Management</h3>
                <div style="display: flex; gap: 10px;">
                    <button class="btn btn-outline">Export Data</button>
                    <button class="btn btn-outline">Import Data</button>
                    <button class="btn btn-outline" style="color: #ff4444; border-color: #ff4444;">Reset All Data</button>
                </div>
            </div>
        </div>
    `;
    ...
    settingsContent.appendChild(settingsData);
...
function createDemoContent() {
    ...
    book1Categories.set(howToArticleId, {
        ...
        parentId: gettingStartedId,
        bookId: bookId1,
        content: 'Knowledge Base is structured hierarchically:1. Books are your main collections2. Books contain Categories for broad topics3. Categories contain Folders, Articles or Files4. Folders can contain more Folders, Articles or FilesTo add new content, click the "Add new" button and select what you want to create.',
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        status: 'published'
    });
    ...
    function renderTreeItem(item, indentLevel = 0, tableBody) {
        ...
        if (item.type === 'category' || item.type === 'folder') {
            ...
            if (bookCategories.has(selectedBookId)) {
                ...
                const childrenExist = Array.from(categoryMap.values()).some(child => {
            ...
            // Only show expand button if there are children
            expandButton = `
                <div class="expandable-control">
                    <div class="expand-icon" ${!hasChildren ? 'style="visibility: hidden;"' : ''}>
            ...
            // Store whether this item has children as a data attribute for later use
            row.setAttribute('data-has-children', hasChildren ? 'true' : 'false');
        }
        ...
        if (item.type === 'article') {
            ...
            // Add a file tag to indicate this is a file, not an article
            status = `
                <span class="status-tag file">File</span>
            `;
        }
        ...
        if (item.type === 'category' || item.type === 'folder') {
            ...
            if (expandIcon) {
                // We don't need to add a click handler directly anymore since we're using event delegation
                // in setupTreeEventListeners. However, we will set up the initial state based on whether 
                // this item should auto-expand.
...
function showSettingsPage() {
    ...
    // Add click event listener for import button
...
/**
 * Exports all application data including localStorage and files from the storage system
 * Creates a ZIP file containing both the storage directory and metadata JSON
 */
function exportAllData() {
    ...
    try {
        ...
        if (typeof JSZip === 'undefined') {
            ...
            showNotification('Error: Required library not available. Please refresh the page.', 'error');
            return;
        }
        
        // Show a notification that export is starting
        showNotification('Creating export package...', 'info');
        ...
        // Add each localStorage item to export data
        keysToExport.forEach(key => {
            const data = localStorage.getItem(key);
            if (data) {
                exportData.localStorage[key] = JSON.parse(data);
            }
        });
        ...
        // Create a storage folder in the ZIP
        const storageFolder = zip.folder('storage');
        
        // Path to the physical storage directory
        const physicalStoragePath = 'src/storage';
        
        // Start zipping the physical storage directory
        zipStorageDirectory(storageFolder, physicalStoragePath)
            .then(() => {
                // Generate and download the ZIP when complete
                generateAndDownloadZip(zip);
            })
            .catch(error => {
                console.error('Error zipping storage directory:', error);
                showNotification('Error creating export package. Some files may be missing.', 'warning');
                // Attempt to generate the ZIP anyway with what we have
                generateAndDownloadZip(zip);
            });
    } catch (error) {
    ...
    /**
     * Zips the physical storage directory and all its contents
     * @param {JSZip} storageFolder - The storage folder in the ZIP
     * @param {string} baseStoragePath - The base path to the storage directory
     * @returns {Promise} - Promise that resolves when all files are processed
     */
    function zipStorageDirectory(storageFolder, baseStoragePath) {
        console.log(`Zipping directory: ${baseStoragePath}`);
        
        // Create a manifest of known directories and files
        const rootDirs = [
            '1F-All Books',
            '2F-Work',
            '3F-Personal',
        ...
        // Add all known subdirectories to ensure we create the full structure
        const knownSubdirs = [
            '1F-All Books/1B-Knowledge Base Guide',
            '1F-All Books/1B-Knowledge Base Guide/1C-Getting Started',
            '1F-All Books/1B-Knowledge Base Guide/2C-General',
            '1F-All Books/1B-Knowledge Base Guide/3C-FAQ',
            '1F-All Books/2B-Project Documentation',
            '1F-All Books/2B-Project Documentation/1C-Requirements',
            '1F-All Books/2B-Project Documentation/2C-Design',
            '6F-Getting Started/1B-User Guide',
            '6F-Getting Started/2B-Developer Resources'
        ];
        
        // Create all directories in the ZIP
        [...rootDirs, ...knownSubdirs].forEach(dir => {
            storageFolder.folder(dir);
            console.log(`Created directory in ZIP: ${dir}`);
        });
        
        // Add sample files as placeholders
        const sampleFiles = [
            {
                path: '1F-All Books/1B-Knowledge Base Guide/1C-Getting Started/1A-Welcome.html',
                content: '<!DOCTYPE html><html><body><h1>Welcome to Knowledge Base</h1><p>This is a sample article.</p></body></html>'
            },
            {
                path: '1F-All Books/1B-Knowledge Base Guide/3C-FAQ/1A-Common Questions.html',
                content: '<!DOCTYPE html><html><body><h1>Frequently Asked Questions</h1><p>This is a sample FAQ article.</p></body></html>'
            }
        ];
        
        // Add sample files to ZIP
        sampleFiles.forEach(file => {
            storageFolder.file(file.path, file.content);
            console.log(`Added sample file to ZIP: ${file.path}`);
        });
        
        // Add files from localStorage data
        return addFilesFromLocalStorage()
            .then(() => {
        ...
        /**
         * Adds files from localStorage data
         * @returns {Promise} - Promise that resolves when all files are processed
         */
        function addFilesFromLocalStorage() {
            return new Promise((resolve, reject) => {
                // Get files data from localStorage
                ...
                try {
                    ...
                    if (!files || !Array.isArray(files)) {
                        console.log('Invalid files data in localStorage');
                        resolve();
                        return;
                    }
                    ...
                    // Filter files that have storage paths
                    const validFiles = files.filter(file => file.storagePath);
                    console.log(`Found ${validFiles.length} files with valid storage paths`);
                    
                    // Process all files
                    const promises = validFiles.map(file => {
                        return addFileToZip(file);
                    });
                    
                    // Wait for all files to be processed
                    Promise.all(promises)
                        .then(() => {
                            console.log('All files from localStorage processed');
                            resolve();
                        })
                        .catch(error => {
                            console.warn('Some files could not be processed:', error);
         ...
         * @param {Object} file - The file object from localStorage
         * @returns {Promise} - Promise that resolves when file is added
         */
        function addFileToZip(file) {
            return new Promise((resolve) => {
                // Normalize the storage path (remove 'storage/' prefix if present)
                const normalizedPath = file.storagePath.replace(/^storage\//, '');
                
                // Create URL for this file
                const fileUrl = chrome.runtime.getURL(`${baseStoragePath}/${normalizedPath}`);
                console.log(`Attempting to fetch file: ${normalizedPath} from ${fileUrl}`);
                
                // Ensure parent directories exist
                const lastSlashIndex = normalizedPath.lastIndexOf('/');
                if (lastSlashIndex > 0) {
                    const dirPath = normalizedPath.substring(0, lastSlashIndex);
                    storageFolder.folder(dirPath);
                }
                ...
                fetch(fileUrl)
                    ...
                    .then(buffer => {
                        // Add file to ZIP with binary content
                        storageFolder.file(normalizedPath, buffer, {
                            binary: true
                        });
                        console.log(`Added file to ZIP: ${normalizedPath}`);
                        resolve();
                    })
                    .catch(error => {
                        console.warn(`Failed to fetch file: ${normalizedPath} - ${error.message}`);
                        // Add a placeholder instead
                        storageFolder.file(normalizedPath, `Content unavailable for ${file.name} - ${new Date().toISOString()}`);
                        resolve();
                    });
            });
        }
     ...
     * @param {JSZip} zip - The ZIP object to generate
     */
    function generateAndDownloadZip(zip) {
        ...
        .then(zipBlob => {
            ...
            downloadLink.download = `knowledge-base-export-${new Date().toISOString().split('T')[0]}.zip`;
        ...
        .catch(error => {
            ...
            showNotification('Error creating ZIP archive. Please try again.', 'error');
        });
    }
}

/**
 * Imports application data from a ZIP file and restores localStorage and file storage
...
function importAllData(file) {
    ...
    try {
        ...
        if (!isZip && !isJson) {
            showNotification('Invalid file format. Please upload a ZIP or JSON file.', 'error');
            return;
        }
        
        // Check if JSZip is available for ZIP files
    ...
    /**
     * Processes the import file based on its type
    ...
    /**
     * Processes a ZIP file import
     * @param {File} file - The ZIP file to import
     */
    function processZipFile(file) {
        showNotification('Processing ZIP archive...', 'info');
        ...
        reader.onload = function(event) {
            try {
                ...
                JSZip.loadAsync(event.target.result)
                    .then(function(zip) {
                        // First, look for metadata.json at the root
                        return zip.file('metadata.json').async('string')
                            .then(function(metadataContent) {
                                ...
                                if (!importData.version || !importData.localStorage) {
                                    throw new Error('Invalid metadata format in ZIP file');
                                }
                                ...
                                return { zip, importData };
                            })
                            .catch(function(error) {
                                console.error('Error processing metadata.json:', error);
                                throw new Error('Could not process metadata in ZIP file');
                            });
                    })
                    .then(function({ zip, importData }) {
                        // Restore localStorage data
                        const localStorage = importData.localStorage;
                        Object.keys(localStorage).forEach(key => {
                            if (localStorage[key]) {
                                window.localStorage.setItem(key, JSON.stringify(localStorage[key]));
                            }
                        });
                        
                        // Process storage folder and restore files from the ZIP
                        return restoreFilesFromZip(zip);
                    })
                    .then(function() {
                        // Refresh the UI
                        refreshAllViews();
                ...
                showNotification('Error importing data. Invalid ZIP format.', 'error');
            }
        };
    ...
    /**
     * Processes a JSON file import (legacy format)
        ...
        reader.onload = function(event) {
            try {
                ...
                if (confirm('This will overwrite your current data. Continue?')) {
                    ...
                    // Process file manifest if available
                    if (importData.fileManifest && window.FileStorageSystem) {
                        console.log(`Found ${importData.fileManifest.length} files in manifest. Note: Actual file content not available in JSON format.`);
                    }
                    
                    // Refresh the UI
                    refreshAllViews();
                    
                    showNotification('Data imported successfully from JSON', 'success');
                }
            } catch (error) {
    ...
    /**
     * Restores files from a ZIP archive to the storage system
     * @param {JSZip} zip - The JSZip instance
     * @returns {Promise} - A promise that resolves when all files are restored
     */
    function restoreFilesFromZip(zip) {
        // First, check if we have a storage folder in the ZIP
        const storageFolder = zip.folder('storage');
        if (!storageFolder) {
            console.warn('No storage folder found in ZIP file');
            return Promise.resolve();
        }
        
        // Get all files in the storage folder
        const zipFiles = [];
        ...
        showNotification(`Restoring ${zipFiles.length} files...`, 'info');
        
        // Array to store promises for all file restorations
        const filePromises = [];
        
        // Get current files from localStorage to update or add to
        let existingFiles = [];
        try {
        ...
        zipFiles.forEach(zipFile => {
            ...
            const filePromise = zipFile.zipEntry.async('arraybuffer')
                .then(content => {
                    // Create full storage path
                    const storagePath = `storage/${zipFile.path}`;
                    
                    // Get filename from path
                    const fileName = zipFile.path.split('/').pop();
                    ...
                    // Check if this file already exists in our records
                    const existingIndex = existingFiles.findIndex(f => 
                        f.storagePath === storagePath || f.name === fileName
                    );
                    ...
                    if (existingIndex >= 0) {
                        ...
                        console.log(`Updated existing file record: ${fileName}`);
                    } else {
                        // Add new file record
                        existingFiles.push(fileInfo);
                        console.log(`Added new file record: ${fileName}`);
                    }
                    
                    // Save the file to the appropriate category structure
                    // In a real implementation, you would use FileStorageSystem to save the file
                    console.log(`Restored file: ${storagePath}`);
                    
                    return Promise.resolve();
                })
                .catch(error => {
                    console.error(`Error restoring file ${zipFile.path}:`, error);
                    return Promise.resolve(); // Continue despite errors
                });
        ...
        // Wait for all file promises to complete, then update localStorage
        return Promise.all(filePromises)
            .then(() => {
                // Save updated files array to localStorage
                localStorage.setItem('files', JSON.stringify(existingFiles));
                console.log(`Saved ${existingFiles.length} file records to localStorage`);
                return Promise.resolve();
            });
         ...
         * @returns {string} - The file type (e.g., 'pdf', 'image', 'document')
         */
    ...
    /**
     * Updates a file record in the files array
     * @param {Object} fileInfo - The file info from the manifest
     * @param {ArrayBuffer} content - The file content
     * @returns {Promise} - A promise that resolves when the file record is updated
     */
    function updateFileRecord(fileInfo, content) {
        // Get the files array from localStorage
        const filesData = localStorage.getItem('files');
        if (!filesData) return Promise.resolve();
        
        const files = JSON.parse(filesData);
        if (!Array.isArray(files)) return Promise.resolve();
        
        // Look for an existing file with the same ID
        const existingIndex = files.findIndex(f => f.id === fileInfo.id);
        
        // If the file exists, update it; otherwise add it
        if (existingIndex >= 0) {
            files[existingIndex] = {
                ...files[existingIndex],
                ...fileInfo,
                ...
                isPrivate: fileInfo.isPrivate || false
            };
        } else {
            files.push({
                ...fileInfo,
...
/**
 * Refreshes all views after data import
 */
function refreshAllViews() {
    // This would reload the sidebar, article viewer, etc.
    console.log("🔄 Refreshing all views after import...");
    
    // Reload the sidebar
    initSidebar();
    
    // Clear article viewer
    const contentContainer = document.getElementById('contentContainer');
    if (contentContainer) {
        contentContainer.innerHTML = '';
    }
    
    // If we were on the home page, reload it
    if (currentPage === 'home') {
        showHomePage();
    }
}
...
function toggleProfileMenu() {
    ...
    upgradeButton.addEventListener('click', () => {
        ...
        const userData = JSON.parse(localStorage.getItem('kb_user') || '{}');
        ...
        const subscription = JSON.parse(localStorage.getItem('kb_subscription') || '{}');
...
function showAdminPage() {
    ...
    pendingRequestsBadge.style.backgroundColor = 'rgba(255, 151, 68, 0.1)';
    pendingRequestsBadge.style.color = 'var(--accent-color)';
    pendingRequestsBadge.style.padding = '3px 10px';
    pendingRequestsBadge.style.borderRadius = '12px';
    pendingRequestsBadge.style.fontSize = '12px';
    pendingRequestsBadge.style.fontWeight = '500';
    pendingRequestsBadge.textContent = '3 Pending';
    
    accessHeader.appendChild(accessTitle);
    accessHeader.appendChild(pendingRequestsBadge);
